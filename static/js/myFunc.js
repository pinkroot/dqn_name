...
function submitFunc() {
  var input_ans = $("#input_text").val();
	...
	if ((test_json.yomi_hiragana == input_ans) || (test_json.yomi_roma == input_ans)){
		$("#result").html("<img src='/static/img/ok.png'>");
		...
		$.post("/problem/answer", {id: test_json._id, state: 0}, function(json){});
	}

	// 回答が間違っていた時の処理
	else {
		$("#result").html("<img src='./img/no.png'><br><p>せいかいは " + test_json.yomi_hiragana + " です");
		$.post("/problem/answer", {id: test_json._id, state: 1}, function(json){});
...
*/
function quiz(){
	$.getJSON("get-quiz", {}, function (json) {
		test_json = json;
